...
export class LLMClient {
  ...
  private buildSystemPrompt(url: string | null, pageText: string | null): string {
    const parts: string[] = [
      ...
      "1. You are an AUTONOMOUS browser agent. When given a task, complete it ENTIRELY using tools.",
      "2. NEVER output text between tool calls. Call all needed tools first, THEN summarize at the end.",
      "3. After navigate: ALWAYS call get_page_elements('inputs') immediately as your next tool call.",
      "4. After getting page elements: use type_text with a selector from the results, or click a button.",
      "5. For type_text: pass placeholder text as selector — it auto-finds the matching input.",
      "6. If a tool fails: call get_page_elements to find the correct selector, then retry.",
      "7. Only produce text AFTER you have completed all tool calls for the task.",
      "",
      "Example task: 'go to X and search for Y'",
      "Correct: navigate('X') → get_page_elements('inputs') → type_text(selector, 'Y') → read_page()",
      "Wrong: navigate('X') → 'I navigated to X, now I will...' (NEVER DO THIS)",
    ];
